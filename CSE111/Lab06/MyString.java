public class MyString{
  ...
  public MyString substring (int start, int end){
    ...
    public int compareTo(String str){
      ...
      int limit = Math.min(ch2.length, ch.length);
      ...
      return ch.length-ch2.length;
    }
    
    public int compareToIgnoreCase(MyString str){
      MyString parameter = str.toLowerCase();
      MyString mainStr = this.toLowerCase();
      int limit = Math.min(parameter.ch.length, mainStr.ch.length);
      
      for (int i=0; i<limit; i++){
        if (parameter.ch[i] != mainStr.ch[i]){
          return mainStr.ch[i] - parameter.ch[i];
        }
      }
      return mainStr.ch.length-parameter.ch.length;
    }
}
